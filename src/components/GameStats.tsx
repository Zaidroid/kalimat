    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full shadow-xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">الإحصائيات</h2>
          <button 
            onClick={handleClose}
          ...
          {stats.distribution.map((count, i) => (
            <div key={i} className="flex items-center">
              ...
              <div className="flex-1 mx-2">
                <div 
                  className={`h-5 ${gameWon && gameState.guesses.length === i + 1 ? 'bg-green-500 dark:bg-green-600' : 'bg-gray-300 dark:bg-gray-600'} rounded`}
        ...
        {gameState.gameOver && (
          <div className="mt-6 text-center">
            <p className="text-gray-700 dark:text-gray-300 mb-2">
              {gameWon ? '🎉 أحسنت! 🎉' : `الكلمة كانت: ${solution}`}
            </p>
            <button
